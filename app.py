...
def signup():
    if request.method == 'POST':
        ...
        # Store pending signup data (NOT creating account yet)
        ...
        # Create a pending_signups table if not exists (with email_otp columns)
        cursor.execute('''
...
def get_site_info(url):
    ...
    if 'amazon' in url_lower:
        ...
        return 'snapdeal', 'INR', '₹'
    elif 'tatacliq' in url_lower or 'tata cliq' in url_lower:
        return 'tatacliq', 'INR', '₹'
...
def scrape_flipkart_price(soup):
    ...
    main_price_area = soup.select_one("div._2DqnCV, div._2B堆积_pV, div._2X_du7")  # Try common main price areas
    if main_price_area:
...
def create_tracker():
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (session['user_id'], url, product_name, current_price, target_price, currency, currency_symbol))
