...
@app.route('/host.html')
def host():
    """Serve the shared host link page"""
    return render_template('host.html')

@app.route('/price-history')
def price_history():
    """Serve the price history graph page"""
    return render_template('price-history.html')

@app.route('/robots.txt')
def robots():
    response = make_response(send_from_directory('static', 'robots.txt'))
...
def signup():
    if request.method == 'POST':
        try:
            ...
            print(f"[SIGNUP] Attempting signup for email: {email}")

            # Phone is optional, so we check required fields separately
            if not all([username, email, password]):
                print(f"[SIGNUP] ERROR: Missing data - username={username}, email={email}, password=***")
                return jsonify({"error": "Missing required fields: username, email, and password are required"}), 400
            
            # Phone can be None or empty string (it's optional)
            if phone == '':
                phone = None
                print(f"[SIGNUP] Phone is empty, setting to None")

            # Validate email format - more permissive regex that supports multi-level TLDs
            email_pattern = r'^[^\s@]+@[^\s@]+\.[^\s@]+$'
            if not re.match(email_pattern, email):
                print(f"[SIGNUP] ERROR: Invalid email format: {email}")
                return jsonify({"error": "Invalid email format"}), 400

            # Validate password strength
            if len(password) < 6:
                print(f"[SIGNUP] ERROR: Password too short")
                return jsonify({"error": "Password must be at least 6 characters"}), 400

            # Check database connection
            print(f"[SIGNUP] Connecting to database...")
            conn = sqlite3.connect(DATABASE)
            cursor = conn.cursor()
            ...
            if not cursor.fetchone():
                print(f"[SIGNUP] ERROR: users table doesn't exist, initializing database...")
                conn.close()
                init_db()
                conn = sqlite3.connect(DATABASE)
                cursor = conn.cursor()
            ...
            print(f"[SIGNUP] Checking if email exists: {email}")
            cursor.execute("SELECT id FROM users WHERE email = ?", (email,))
            if cursor.fetchone():
                conn.close()
                print(f"[SIGNUP] ERROR: Email already exists")
                return jsonify({"error": "Email already exists. Please login instead."}), 409
            ...
            if cursor.fetchone():
                ...
                print(f"[SIGNUP] Deleting old pending signup for: {email}")
            ...
            print(f"[SIGNUP] Creating new user account...")
            conn = sqlite3.connect(DATABASE)
            cursor = conn.cursor()
            ...
            print(f"[SIGNUP] Password hashed successfully, length: {len(hashed_password)}")
            ...
            if created_user:
                print(f"[SIGNUP] VERIFIED: User exists in database: {created_user}")
            else:
                print(f"[SIGNUP] WARNING: User ID {user_id} not found after insert!")
            ...
            print(f"[SIGNUP] DATABASE INTEGRITY ERROR: {e}")
            return jsonify({"error": "An account with this email already exists."}), 409
        except sqlite3.OperationalError as e:
            print(f"[SIGNUP] DATABASE OPERATIONAL ERROR: {e}")
...
def get_site_info(url):
    ...
    # Tata CLiQ
    if 'tatacliq' in url_lower or 'tata' in url_lower and ('cliq' in url_lower or 'tiq' in url_lower):
        return 'tatacliq', 'INR', '₹'
    
    # Reliance Digital
    if 'reliancedigital' in url_lower or 'reliance' in url_lower and 'digital' in url_lower:
        return 'reliancedigital', 'INR', '₹'
    
    # Croma (Reliance owned)
    ...
    # JioMart
    if 'jiomart' in url_lower or 'jiomart' in url_lower:
        return 'jiomart', 'INR', '₹'
    ...
    # AliExpress
    if 'aliexpress' in url_lower or 'aliexpress' in url_lower:
    ...
    # H&M
    if 'hm' in url_lower or 'hm.com' in url_lower:
        return 'hm', 'USD', '$'
    ...
    # Apple
    if 'apple' in url_lower or 'apple.com' in url_lower:
        if 'apple.in' in url_lower:
            return 'apple', 'INR', '₹'
        return 'apple', 'USD', '$'
    
    # Mi/Xiaomi
    if 'mi.com' in url_lower or 'xiaomi' in url_lower or 'mi-store' in url_lower:
        return 'mi', 'INR', '₹'
    ...
    # Country code detection for Indian domains
...
def scrape_reliance_digital_price(soup):
    ...
    return generic_scrape_price(soup)

def scrape_croma_price(soup):
...
def scrape_firstcry_price(soup):
...
def scrape_pepperfry_price(soup):
...
def scrape_urbanladder_price(soup):
...
def scrape_bigbasket_price(soup):
...
def scrape_vijaysales_price(soup):
...
# ==================== GLOBAL SITES SCRAPING FUNCTIONS ====================

def scrape_aliexpress_price(soup):
...
def scrape_samsung_price(soup):
    """Scrape price from Samsung Store pages"""
    selectors = [
        "span[itemprop='price']",
        "div.price",
        "span.final-price",
    ...
    for selector in selectors:
        ...
        if price_element:
            text = price_element.get_text().strip()
            price = parse_price(text)
            if price and 500 < price < 1000000:
                return price
    
    return generic_scrape_price(soup)

def scrape_apple_price(soup):
    """Scrape price from Apple Store pages"""
    selectors = [
        "span[itemprop='price']",
        "div.price",
        "span.final-price",
        "[class*='price']",
        "[class*='Price']",
        "span.as-price-current-price",
        "div.as-price-current-price"
    ]
...
def scrape_mi_price(soup):
    """Scrape price from Mi Store pages"""
    selectors = [
        "span[itemprop='price']",
        "div.price",
        "span.final-price",
...
def scrape_oneplus_price(soup):
    """Scrape price from OnePlus Store pages"""
    selectors = [
        "span[itemprop='price']",
        "div.price",
        "span.final-price",
...
def generic_scrape_price(soup):
    """Generic price scraping fallback for unknown sites"""
...
def get_price():
    ...
    try:
        ...
        if site == 'amazon':
            ...
            price = scrape_croma_price(soup)
        elif site == 'nykaa':
            price = scrape_nykaa_price(soup)
        elif site == 'firstcry':
            price = scrape_firstcry_price(soup)
        elif site == 'pepperfry':
            price = scrape_pepperfry_price(soup)
        elif site == 'urbanladder':
            price = scrape_urbanladder_price(soup)
        elif site == 'bigbasket':
            price = scrape_bigbasket_price(soup)
        elif site == 'jiomart':
            price = scrape_jiomart_price(soup)
        elif site == 'vijaysales':
            price = scrape_vijaysales_price(soup)
        elif site == 'aliexpress':
            price = scrape_aliexpress_price(soup)
        elif site == 'walmart':
            price = scrape_walmart_price(soup)
        elif site == 'bestbuy':
            price = scrape_bestbuy_price(soup)
        elif site == 'target':
            price = scrape_target_price(soup)
        elif site == 'etsy':
            price = scrape_etsy_price(soup)
        elif site == 'newegg':
            price = scrape_newegg_price(soup)
        elif site == 'shein':
            price = scrape_shein_price(soup)
        elif site == 'zara':
            price = scrape_zara_price(soup)
        elif site == 'hm':
            price = scrape_hm_price(soup)
        elif site == 'adidas':
            price = scrape_adidas_price(soup)
        elif site == 'nike':
            price = scrape_nike_price(soup)
        elif site == 'samsung':
            price = scrape_samsung_price(soup)
        elif site == 'apple':
            price = scrape_apple_price(soup)
        elif site == 'mi':
            price = scrape_mi_price(soup)
        elif site == 'oneplus':
            price = scrape_oneplus_price(soup)
        else:
            # Fallback to generic scraping for unknown sites
            price = generic_scrape_price(soup)
...
def health_check():
    """Health check endpoint to verify database connection and system status"""
    try:
...
# ==================== HTTP HEADERS FOR CRAWLERS ====================

@app.after_request
def add_crawler_headers(response):
    """Add HTTP headers to improve crawler compatibility and security"""
    # Security headers
    ...
    # CORS headers already handled by Flask-CORS
    return response
...
def sitemap():
    ...
    pages = [
        ...
        {'loc': f'{base_url}/login', 'changefreq': 'weekly', 'priority': '0.8'},
        {'loc': f'{base_url}/signup', 'changefreq': 'weekly', 'priority': '0.8'},
        {'loc': f'{base_url}/forgot-password', 'changefreq': 'monthly', 'priority': '0.5'},
        {'loc': f'{base_url}/reset-password', 'changefreq': 'monthly', 'priority': '0.5'},
        {'loc': f'{base_url}/error', 'changefreq': 'yearly', 'priority': '0.3'},
    ]
...
# ==================== CRAWLER VERIFICATION ENDPOINT ====================

@app.route('/api/crawler-verify', methods=['GET'])
def crawler_verify():
    """Dedicated endpoint for search engine crawlers to verify server status"""
    return jsonify({
        "status": "ok",
        "message": "Server is operational and accessible to crawlers",
        "timestamp": datetime.now().isoformat(),
        "version": "2.0.0",
        "site": "pricealerter.in"
    }), 200

@app.route('/google<hash>.html', methods=['GET'])
def google_site_verification(hash):
    """Google Search Console site verification endpoint"""
    # Replace 'YOUR_HASH' with your actual Google Search Console verification hash
    # Example: google1234567890abcdef.html
    return jsonify({
        "message": "Google Site Verification",
        "hash": hash
    }), 200

@app.route('/api/ping', methods=['GET'])
def ping():
    """Lightweight ping endpoint for monitoring and connectivity checks"""
    return jsonify({
        "status": "ok",
        "timestamp": datetime.now().isoformat(),
        "uptime": "operational"
...
def not_found(error):
    """Handle 404 errors with proper status and helpful message"""
    return render_template('error.html', error="Page not found"), 404

@app.errorhandler(500)
def internal_error(error):
    """Handle 500 errors with retry suggestions"""
    response = jsonify({
...
class TimeoutMiddleware:
    """Middleware to handle connection timeouts gracefully"""
...
# Apply timeout middleware (optional, can be enabled for production)
